...
private fun convertToCorrectHand(strategy: List<String>): Hand {
...
private fun play(hands: Pair<Hand, Hand>): Int {
...
private fun isDrawingHand(hands: Pair<Hand, Hand>) = listOf(
        Pair(Hand.ROCK, Hand.ROCK),
        Pair(Hand.PAPER, Hand.PAPER),
        Pair(Hand.SCISSOR, Hand.SCISSOR)
).contains(hands)

private fun getPointsForResult(left: Boolean, right: Boolean): Int {
    if (right && !left) {
        return 6
    }
    if (right == left) {
...
private fun getPointForHand(hand: Hand): Int {
    return when(hand) {
        Hand.ROCK -> 1
        Hand.PAPER -> 2
        Hand.SCISSOR -> 3
